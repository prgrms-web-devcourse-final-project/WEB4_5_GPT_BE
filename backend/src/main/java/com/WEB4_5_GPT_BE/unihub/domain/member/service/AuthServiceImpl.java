...
public class AuthServiceImpl implements AuthService {
  ...
  private void validateLoginFailLimit(String email) {
    ...
    int failCount = Optional.ofNullable(failCountStr).map(Integer::parseInt).orElse(0);
  ...
  private MemberLoginResponse generateLoginResponse(Member member) {
  ...
  public void logout(HttpServletRequest request, HttpServletResponse response) {
    ...
    if (accessToken == null) {
      throw new UnihubException("401", "인증이 필요합니다.");
    }

    Long memberId = authTokenService.getMemberIdFromToken(accessToken);
    if (memberId == null) {
      throw new UnihubException("401", "유효하지 않은 형식.");
    }
  ...
  @Override
  @Transactional
  public MemberLoginResponse refreshAccessToken(
      HttpServletRequest request, HttpServletResponse response) {
    String refreshToken = rq.getRefreshToken();
    if (refreshToken == null) {
      throw new UnihubException("401", "Refresh Token이 존재하지 않습니다.");
    }
    ...
    if (memberId == null) {
      throw new UnihubException("400", "잘못된 refreshToken 형식입니다.");
    }
